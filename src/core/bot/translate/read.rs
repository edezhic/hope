...
impl Bot {
    pub fn read(&self, pieces: &mut Peekable<UWordBounds<'_>>) -> Result<Lexeme> {
        let lexeme = match *pieces.peek().unwrap() {
            ...
            piece if self.vocab.val_id.is_match(piece) => Lexeme::Item(Value::Id(self.collect_id(pieces)?)),
            piece if self.vocab.val_number.is_match(piece) => Lexeme::Item(Value::Number(self.collect_number(pieces)?)),
            piece if self.vocab.val_seal.is_match(piece) => Lexeme::Item(Value::Seal(self.collect_seal(pieces)?)),
            piece if self.vocab.val_text.is_match(piece) => Lexeme::Item(Value::Text(self.collect_text(pieces)?)),
            piece if self.vocab.val_time.is_match(piece) => Lexeme::Item(Value::Time(self.collect_time(pieces)?)),
            piece if self.vocab.val_version.is_match(piece) => Lexeme::Item(Value::Version(self.collect_version(pieces)?)),
            piece => {
                if let Some(fact) = self.lookup_fact(pieces) { 
                    Lexeme::Item(Value::Fact(fact)) 
    ...
    fn lookup_keyword(&self, pieces: &mut Peekable<UWordBounds<'_>>) -> Option<Token> {
        let keyword = match *pieces.peek().unwrap() {
            ...
            piece if self.vocab.case_identical.is_match(piece) => Some(Token::Case(Case::Identical)),
            ...
            piece if self.vocab.op_define.is_match(piece) => Some(Token::Op(Op::Define)),
            ...
            piece if self.vocab.mod_selection.is_match(piece) => Some(Token::Mod(Modifier::Selection)),
            piece if self.vocab.mod_targeting.is_match(piece) => Some(Token::Mod(Modifier::Targeting)),
            _ => None,
        };
